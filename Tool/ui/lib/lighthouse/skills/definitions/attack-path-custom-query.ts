...
This skill provides openCypher syntax guidance and Cartography schema knowledge for writing graph queries against AzCSPM's cloud infrastructure data.
...
4. **Write the query**: Compose the openCypher query following all rules in this skill:
   ...
   - Include \`ProwlerFinding\` OPTIONAL MATCH (see Include AzCSPM Findings)
...
### Include AzCSPM Findings

Always include AzCSPM findings to enrich results with security context:
...
### AzCSPM-Specific Labels

| Label | Description |
|-------|-------------|
| \`ProwlerFinding\` | AzCSPM finding node with \`status\`, \`provider_uid\`, \`severity\` properties |
...
3. **Include AzCSPM findings**: Always add the \`OPTIONAL MATCH\` for \`ProwlerFinding\` nodes.
